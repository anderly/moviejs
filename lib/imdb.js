...
var imdb = function() {
	...
	this.findById = function(id, callback) {
		this._getCacheWithDefault(id, function(err, movie) {
			if (!err) {
				if (movie) {
					return callback(null, movie);
				} else {
					return self._getResponse(id, function(err, movie) {
						return callback(null, movie); 
					});
				}
			} else {
	...
	this.search = function(search, callback) {
		...
		this._getCacheWithDefault(escape(search), function(err, results) {
			if (!err) {
				if (results) {
					return callback(null, results);
				} else {
					return self._getResults(search, function(err, results) {
						return callback(null, results); 
					});
				}
			} else {
	...
	this._getCacheWithDefault = function(id, callback) {
	...
	this._getResponse = function(id, callback) {
	...
	this._getResults = function(search, callback) {
